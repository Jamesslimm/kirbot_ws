...
def generate_launch_description() -> LaunchDescription:
    ...
    dwm_transform = Node(
        ...
        name="dwm_transform",
        parameters=[{'position_cov': [0.0, 0.0, 0.0, 3.45, 0.0, 0.0, 0.43, 3.43, 0.0]}],
        remappings=[("input/tag_position", tag_topic_value)],
    )

    anchor_visualizer_node = Node(
        package="dwm1001_visualization",
        executable="anchor_visualizer",
        name="anchor_visualizer",
        parameters=[
            {"initiator_anchor_position": "0.0,0.0,0.0"},
            {"anchor_positions": "3.45, 0.0, 0.0; 0.43, 3.43, 0.0"},
        ],
    )

    return LaunchDescription(
        [tag_id_arg, dwm_transform, anchor_visualizer_node]
    )