...
class PassiveTagNode(Node):
    def __init__(self) -> None:
        ...
        self.transformed_publishers_dict = dict()
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter("num_samples", 8, num_samples_decriptor)

    def timer_callback(self):
        try:
            tag_label, tag_position = self.dwm_handle.wait_for_position_report()
            self.get_logger().info(f"Received position for {tag_label}: {tag_position}")
        ...
        if tag_position.quality < 10:
                self.get_logger().warn(f"Low quality ({tag_position.quality}) for tag {tag_label}. Skipping this reading.")
                return
        ...
        if tag_label not in self.publishers_dict:
            self.get_logger().info(f"Discovered new active tag '{tag_label}'. Creating publishers.")
            tag_topic = f'~/output/{tag_label}'
            transformed_tag_topic = f'~/output/{tag_label}_transformed'
            
            self.publishers_dict[tag_label] = self.create_publisher(
                PointStamped, tag_topic, 1
            )
            self.transformed_publishers_dict[tag_label] = self.create_publisher(
                PointStamped, transformed_tag_topic, 1
            )
        ...
        if len(self.position_buffer[tag_label]) == self.num_samples:
            ...
            # Transformed position message with modified y-coordinate
            transformed_msg = PointStamped()
            transformed_msg.header.stamp = time_stamp
            transformed_msg.header.frame_id = "map"
            # transformed_msg.point.x = round(y_avg, 3)
            # transformed_msg.point.y = round(-x_avg + 1, 3)
            transformed_msg.point.x = round(x_avg, 3)
            transformed_msg.point.y = round(-(y_avg -0.5), 3)
            
            transformed_msg.point.z = round(z_avg, 3)

            self.transformed_publishers_dict[tag_label].publish(transformed_msg)
